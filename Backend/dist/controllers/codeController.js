...
exports.importCodes = importCodes;
exports.verifyLimitedCode = verifyLimitedCode;
exports.verifyPriveCode = verifyPriveCode;
exports.getBatchHistory = getBatchHistory;
exports.deleteBatch = deleteBatch;
exports.downloadCodeBatch = downloadCodeBatch;
...
async function generateCodes(req, res) {
    try {
        ...
        const lastCodeInYear = (await Code_1.Code.findOne({
            code: { $regex: `^${prefix}` },
        })
            .sort({ code: -1 })
            .lean()
        ...
        const latestBatchDoc = (await Code_1.Code.findOne()
            .sort({ batch: -1 })
            .lean()
            .exec());
        ...
        const existingCodes = await Code_1.Code.find({ code: { $in: generatedCodes } })
            .lean()
            .exec();
        if (existingCodes.length > 0) {
            return res
                .status(500)
                .json({
                error: "Duplicate code generation attempt failed. Please try again.",
            });
        }
        ...
        await Code_1.Code.insertMany(docs, { ordered: false });
...
async function importCodes(req, res) {
    try {
        ...
        if (!Array.isArray(codes) || codes.length === 0) {
            ...
            const filePath = path_1.default.join(__dirname, "..", "..", "data", "codes-batch-1.json");
            if (!fs_1.default.existsSync(filePath)) {
                return res
                    .status(400)
                    .json({
                    error: "'codes' must be provided in body or Backend/data/codes-batch-1.json must exist",
                });
            }
            ...
            try {
                ...
                if (!Array.isArray(parsed)) {
                    return res
                        .status(400)
                        .json({ error: "codes JSON must be an array of strings" });
                }
                codes = parsed;
            }
            catch (err) {
                return res
                    .status(400)
                    .json({ error: "Failed to parse codes JSON file" });
            }
        }
        ...
        const existing = await Code_1.Code.find({ code: { $in: unique } })
            .lean()
            .exec();
...
async function verifyLimitedCode(req, res) {
    try {
        const { code } = req.body;
        if (!code) {
            return res.status(400).json({ error: "Code is required" });
        }
        const codeDoc = await Code_1.Code.findOne({
            code: code,
        ...
        if (!codeDoc) {
            return res
                .status(404)
                .json({ error: "Invalid or already used code.", ok: false });
        }
...
async function verifyPriveCode(req, res) {
    try {
        const { code } = req.body;
        if (!code) {
            return res.status(400).json({ error: "Code is required" });
        ...
        const authReq = req;
        if (authReq.user?.sub) {
            await Code_1.Code.updateOne({ _id: codeDoc._id }, { $set: { usedBy: authReq.user.sub } });
            await User_1.User.updateOne({ _id: authReq.user.sub }, { $set: { lastPriveCodeVerifiedAt: new Date() } });
        }
        return res.json({ ok: true });
    }
    catch (err) {
...
async function getBatchHistory(req, res) {
    try {
        const history = await Code_1.Code.aggregate([
            {
...
async function deleteBatch(req, res) {
    try {
        ...
        const result = await Code_1.Code.updateMany({ batch: batchNum }, { $set: { isDeleted: true } });
...
async function downloadCodeBatch(req, res) {
    try {
        const { batch } = req.params;
        const batchNum = parseInt(batch, 10);
        if (isNaN(batchNum)) {
        ...
        const codes = await Code_1.Code.find({ batch: batchNum, isDeleted: { $ne: true } })
            .lean()
            .exec();
        if (!codes.length) {
            return res
                .status(404)
                .json({ error: "Batch not found or has been deleted" });
        }
        const doc = new pdfkit_1.default({ margin: 50, bufferPages: true });
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename=dlaven-codes-batch-${batchNum}.pdf`);
        doc.pipe(res);
        doc
            .fontSize(20)
            .font("Helvetica-Bold")
            .text("D'LAVÉN", 50, 57, { align: "left" });
        doc
            .fontSize(10)
            .font("Helvetica")
            .text("Unique Code Report", 50, 80, { align: "left" });
        doc
            .fontSize(10)
            .font("Helvetica-Bold")
            .text(`Batch Number: ${batchNum}`, 50, 57, { align: "right" });
        doc
            .fontSize(10)
            .font("Helvetica")
            .text(`Date Generated: ${new Date(codes[0].createdAt).toLocaleDateString()}`, 50, 72, { align: "right" });
        doc.moveDown(3);
        const tableTop = 150;
        const itemCodeX = 50;
        const serialNoX = 350;
