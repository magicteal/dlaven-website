...
export async function generateCodes(req: Request, res: Response) {
  try {
    ...
    const lastCodeInYear = (await Code.findOne({
      code: { $regex: `^${prefix}` },
    })
      .sort({ code: -1 })
      .lean()
    ...
    const latestBatchDoc = (await Code.findOne()
      .sort({ batch: -1 })
      .lean()
      .exec()) as ICode | null;
    ...
    const existingCodes = await Code.find({ code: { $in: generatedCodes } })
      .lean()
      .exec();
...
export async function importCodes(req: Request, res: Response) {
  try {
    ...
    if (!Array.isArray(codes) || codes.length === 0) {
      ...
      if (!fs.existsSync(filePath)) {
        return res
          ...
          .json({
            error:
              "'codes' must be provided in body or Backend/data/codes-batch-1.json must exist",
          });
      }
    ...
    const existing = await Code.find({ code: { $in: unique } })
      .lean()
      .exec();
  ...
  try {
    ...
    if (authReq.user?.sub) {
      ...
      await User.updateOne(
        { _id: authReq.user.sub },
        { $set: { lastPriveCodeVerifiedAt: new Date() } }
      );
    }
...
export async function downloadCodeBatch(req: Request, res: Response) {
  try {
    ...
    const codes = await Code.find({ batch: batchNum, isDeleted: { $ne: true } })
      .lean()
      .exec();
    if (!codes.length) {
      return res
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        `Date Generated: ${new Date(codes[0].createdAt).toLocaleDateString()}`,
        50,
        72,
        { align: "right" }
      );
