        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14">
            ...
            <div className="grid grid-cols-3 gap-3 order-2 lg:order-1">
              {gallery.map((img, i) => (
                <div key={i} className="bg-gray-100 aspect-square overflow-hidden">
                  <Image src={img} alt={`${product.name} ${i + 1}`} width={600} height={600} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
