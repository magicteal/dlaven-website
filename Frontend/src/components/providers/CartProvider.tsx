...
type CartContextValue = {
  ...
  update: (productSlug: string, quantity: number, size?: string) => Promise<void>;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const update = async (productSlug: string, quantity: number, size?: string) => {
  ...
  const count = useMemo(() => (cart?.items?.reduce((sum, i) => sum + i.quantity, 0) ?? 0), [cart]);
  const subtotal = useMemo(() => (cart?.items?.reduce((sum, i) => sum + i.price * i.quantity, 0) ?? 0), [cart]);

  const value = useMemo(() => ({ cart, loading, add, update, remove, refresh, count, subtotal }), [cart, loading, count, subtotal]);
