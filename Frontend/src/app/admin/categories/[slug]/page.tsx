...
export default function AdminEditCategoryPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await requestAdmin<{ item: { slug: string; name: string; imageSrc: string; imageAlt: string } }>(`/api/categories/${encodeURIComponent(slugParam)}`);
      ...
      <form className="mt-4 max-w-lg" onSubmit={onSave}>
        ...
        <label className="block mb-2">Image Src
          ...
          <div className="mt-2">
            ...
            {imageSrc && (
              <div className="mt-2 w-32 h-32 border border-black/10 overflow-hidden relative">
