...
export default function AdminEditProductPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await requestAdmin<{ item: Product }>(`/api/products/${params.slug}`);
        const p = data.item;
        setForm({
          slug: p.slug,
          name: p.name,
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      ...
      await requestAdmin(`/api/products/${params.slug}`, { method: "PATCH", body: JSON.stringify(body) });
    ...
    <div className="max-w-2xl">
      ...
      {loadingItem ? (
        ...
        <form onSubmit={onSave} className="mt-6 grid grid-cols-1 gap-4">
          ...
          <Input label="Images (comma-separated URLs)" value={form.images} onChange={(v) => setForm((p) => ({ ...p, images: v }))} />
          <div>
