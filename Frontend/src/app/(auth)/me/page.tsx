          ...
          <div className="mt-6 grid gap-6 max-w-3xl">
            <section className="border border-black/10 p-4 sm:p-6">
              ...
              <div className="mt-4 flex gap-3">
                <Link href="/profile" className="px-4 py-2 border border-black text-sm uppercase tracking-wider hover:bg-black hover:text-white">Edit Profile</Link>
                <Link href="/forgot-password" className="px-4 py-2 border border-black text-sm uppercase tracking-wider hover:bg-black hover:text-white">Reset Password</Link>
              </div>
            </section>
            ...
            <section className="border border-black/10 p-4 sm:p-6">
              <div className="flex items-center justify-between">
                ...
                <Link href="/profile" className="text-xs px-3 py-1 border border-black uppercase tracking-wider hover:bg-black hover:text-white">Manage</Link>
              </div>
              {addresses === null ? (
                ...
                <div className="mt-3">
                  ...
                  <Link href="/profile" className="mt-3 inline-block px-3 py-2 border border-black text-xs uppercase tracking-wider hover:bg-black hover:text-white">Add Address</Link>
                </div>
              ) : (
                  ...
                  {addresses.map((a) => (
                    <div key={a.id} className="border border-black/10 p-3 sm:p-4 flex items-start gap-3">
                      ...
                      <div className="flex flex-col gap-2">
                        <Link href="/profile" className="px-3 py-1 border border-black text-xs uppercase tracking-wider hover:bg-black hover:text-white">Edit</Link>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="border border-black/10 p-4 sm:p-6">
              ...
              {orders === null ? (
                <p className="mt-2 text-sm text-black/60">Loading orders…</p>
              ) : orders.length === 0 ? (
                <p className="mt-2 text-sm text-black/60">You have no recent orders.</p>
              ) : (
                <div className="mt-3 space-y-3">
                  {orders.map((o) => (
                    <div key={String(o._id || o.id)} className="border border-black/10 p-3 text-sm">
                      <div className="flex items-center justify-between">
                        <div className="font-medium">Order {String(o._id || o.id)}</div>
                        <div className="uppercase text-xs tracking-wider">{o.status}</div>
                      </div>
